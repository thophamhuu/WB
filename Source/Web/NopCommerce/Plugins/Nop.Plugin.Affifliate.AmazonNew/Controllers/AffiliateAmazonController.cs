...
namespace Nop.Plugin.Affiliate.Amazon.Controllers
{
    ...
    public class AffiliateAmazonController : BasePluginController
    {
        ...
        private readonly int _source = 2;
        ...
        public ActionResult Accounts()
        {
            var storeScope = this.GetActiveStoreScopeConfiguration(_storeService, _workContext);
            var amazonSettings = _settingService.LoadSetting<AffiliateAmazonSettings>(storeScope);
            var model = amazonSettings.ListAccounts ?? new List<AffiliateAmazonAccount>();
            return Json(model, JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
        public ActionResult CreateAccount(string AssociateTag, string AccessKeyID, string SecretKey)
        {
            var storeScope = this.GetActiveStoreScopeConfiguration(_storeService, _workContext);
            var amazonSettings = _settingService.LoadSetting<AffiliateAmazonSettings>(storeScope);
        ...
        [HttpPost]
        public ActionResult DeleteAccount(string AssociateTag, string AccessKeyID, string SecretKey)
        {
            var storeScope = this.GetActiveStoreScopeConfiguration(_storeService, _workContext);
            var amazonSettings = _settingService.LoadSetting<AffiliateAmazonSettings>(storeScope);
        ...
        public ActionResult SyncCategory(string browseNodeID = null)
        {
            var storeScope = this.GetActiveStoreScopeConfiguration(_storeService, _workContext);
            var amazonSettings = _settingService.LoadSetting<AffiliateAmazonSettings>(storeScope);
            _amazonService.SyncCategory(amazonSettings, browseNodeID);
        ...
        public ActionResult LoadBrowseNode(int categoryId = 0)
        {
            ...
            if (category == null)
                return Json(model, JsonRequestBehavior.AllowGet);
            var categoryMaps = _categoryMapRepo.TableNoTracking.Where(x => x.CategoryId == categoryId && x.SourceId == (int)_source);
        ...
        public ActionResult SyncProducts(ProductParameter model)
        {
            ...
            {
                _amazonService.SyncProducts(storeScope, model.CategoryID, model.Keywords, SyncProperties.All);
            }
        ...
        public ActionResult SyncProduct(ProductParameter model)
        {
            ...
            if (amazonSettings.ListAccounts.Count() > 0)
            {
                _amazonService.SyncProduct(storeScope, model.Id, syncProperties);
            }
